...
export const WinningProductsPage: FC = () => {
    ...
    // --- MANAGE THE SELECTED PRODUCT STATE ---
    ...
    return (
        <>
            <main className="flex-1 overflow-y-auto p-4 md:p-8">
                ...
                {/* Filter Bar (No changes needed here) */}
                <div className="sticky top-0 bg-[#000000]/80 backdrop-blur-sm py-4 my-6 rounded-xl z-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                        <div className="relative">
                            <FaSearch className="absolute top-1/2 left-4 -translate-y-1/2 text-neutral-400" />
                            <input type="text" placeholder="Rechercher par mot-clé..." className="w-full pl-10 pr-4 py-3 rounded-lg border-0 bg-[#1a1a1a] text-sm text-white placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-[#f97316]" onChange={(e) => handleFilterChange('keyword', e.target.value)} />
                        </div>
                        <div className="relative">
                            <FaTag className="absolute top-1/2 left-4 -translate-y-1/2 text-neutral-400" />
                            <select className="w-full appearance-none pl-10 pr-4 py-3 rounded-lg border-0 bg-[#1a1a1a] text-sm text-white focus:outline-none focus:ring-2 focus:ring-[#f97316]" onChange={(e) => handleFilterChange('category', e.target.value)} disabled={isLoadingCategories}>
                        ...
                        <div className="relative flex items-center gap-2">
                            ...
                            <input type="number" placeholder="Min" className="w-1/2 pl-10 pr-4 py-3 rounded-lg border-0 bg-[#1a1a1a] text-sm text-white placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-[#f97316]" onChange={(e) => handleFilterChange('minPrice', e.target.value)} />
                            <input type="number" placeholder="Max" className="w-1/2 py-3 px-4 rounded-lg border-0 bg-[#1a1a1a] text-sm text-white placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-[#f97316]" onChange={(e) => handleFilterChange('maxPrice', e.target.value)} />
                        </div>
                        <div className="relative">
                            <FaSortAmountDown className="absolute top-1/2 left-4 -translate-y-1/2 text-neutral-400" />
                            <select className="w-full appearance-none pl-10 pr-4 py-3 rounded-lg border-0 bg-[#1a1a1a] text-sm text-white focus:outline-none focus:ring-2 focus:ring-[#f97316]" value={filters.sortBy} onChange={(e) => handleFilterChange('sortBy', e.target.value)}>
                        ...
                        <button className="flex items-center justify-center gap-2 rounded-lg bg-[#f97316] text-sm font-semibold text-white hover:opacity-90 transition-opacity disabled:opacity-50 disabled:cursor-wait" disabled={isFetching}>
                ...
                {meta && meta.totalPages > 1 && (
                    <div className="flex justify-center items-center gap-4 mt-10">
                        <button onClick={() => handleFilterChange('page', meta.page - 1)} disabled={meta.page <= 1} className="px-4 py-2 rounded-lg bg-[#1a1a1a] text-sm font-semibold text-white hover:bg-[#f97316] disabled:opacity-50 disabled:cursor-not-allowed">
                            Précédent
                        </button>
                        <span className="text-sm text-neutral-400">Page {meta.page} sur {meta.totalPages}</span>
                        <button onClick={() => handleFilterChange('page', meta.page + 1)} disabled={meta.page >= meta.totalPages} className="px-4 py-2 rounded-lg bg-[#1a1a1a] text-sm font-semibold text-white hover:bg-[#f97316] disabled:opacity-50 disabled:cursor-not-allowed">
                            Suivant
                        </button>
                    </div>
                )}
